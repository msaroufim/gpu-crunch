...
export type CardRole = 'Setup' | 'Protect' | 'Timing' | 'Finisher'
...
export const effectRules: Record<EffectId, { name: string; text: string; broken?: boolean }> = {
  shield: { name: 'Shield', text: 'Local buff: this card can be built through suit-lock events.', broken: true },
...
export const cardRole = (card: Card): CardRole => {
  ...
  if (card.effect === 'shield') return 'Protect'
  return 'Setup'
}
...
export const effectiveCost = (card: Card, event?: EventCard): ResourceMap => {
  ...
  if (event?.blockedSuits?.includes(card.suit) && card.effect !== 'shield') {
...
export const CARDS: Card[] = [
  ...
  c('cowos-expansion', 'CoWoS Expansion', 'Fabrication', 3, 'late', 'Capex turns into slots if you can wait long enough.', { money: 4, energy: 1 }, { compute: 3 }, { compute: 2 }, { capacity: 4, grid: 1 }, 4, 'package', 'shield'),
  ...
  c('earthquake-insurance', 'Foundry Earthquake Insurance', 'Risk', 1, 'early', 'You cannot stop the quake, but you can buy resilience.', { money: 1 }, { influence: 1 }, { money: 1 }, { policy: 1, capacity: 1 }, 1, 'risk', 'shield'),
  c('port-strike-buffer', 'Port Strike Buffer', 'Risk', 1, 'early', 'Inventory is inefficient until it saves you.', { money: 2 }, { compute: 1 }, undefined, { capacity: 1, moat: 1 }, 1, 'risk', 'shield'),
  ...
  c('infiniband-switch', 'InfiniBand Switch', 'Cluster', 2, 'mid', 'Latency is a resource if you can monopolize it.', { money: 3, energy: 1 }, undefined, { compute: 1 }, { capacity: 2, moat: 2 }, 3, 'network', 'shield'),
